...
class Ontology(object):
    ...
    def calculate_ic(self, annots):
        ...
        for x in annots:
            cnt.update(x)
        self.ic = {}
        for t_id, n in cnt.items():
            parents = self.get_parents(t_id)
            if len(parents) == 0:
                min_n = n
            else:
                min_n = min([cnt[x] for x in parents])
            self.ic[t_id] = math.log(min_n / n, 2)
    
    def get_ic(self, t_id):
        if self.ic is None:
            raise Exception('Not yet calculated')
        if t_id not in self.ic:
            return 0.0
        return self.ic[t_id]
    ...
    def get_children(self, term_id):
        return self.ont[term_id]['children']
